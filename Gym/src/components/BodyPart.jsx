...
const BodyPart = ({ item, setBodyPart, bodyPart }) => (
  <Stack
    ...
    sx={bodyPart === item ? { borderTop: '4px solid #FFBE0B', background: 'black', borderBottomLeftRadius: '20px', width: '270px', height: '282px', cursor: 'pointer', gap: '47px' } : { background: 'black', borderBottomLeftRadius: '20px', width: '270px', height: '282px', cursor: 'pointer', gap: '47px' }}
    onClick={() => {
      setBodyPart(item);
      window.scrollTo({ top: 1800, left: 100, behavior: 'smooth' });
    }}
